...
# Writes /etc/nixos/flake.nix from the given list of dotted module paths.
# Called once normally; called twice when an Apple Silicon + gaming host
# needs a binfmt-bootstrap pre-stage (once without the gaming-asahi module,
# then again with it).
write_flake_nix() {
  {
    cat <<EOF
...
# Apple Silicon + gaming first-run binfmt pre-stage. When apple-silicon's
# boot.binfmt.emulatedSystems flips for the first time, the running kernel
# has no qemu-x86_64 handler yet and the running nix-daemon has no
# x86_64-linux in extra-platforms — so any allowSubstitutes=false wrapper
# in the gaming closure (buildFHSEnv gsettings-schemas-directory, steam
# *-wrapped, zoom-us *-init) tries to build locally and fails before we
# ever get to activate the new config. Chicken-and-egg.
#
# Workaround: build a stage-1 system that includes binfmt but NOT the
# gaming-asahi role. `switch` activates binfmt in the running kernel and
# restarts nix-daemon with the updated extra-platforms — after which the
# main rebuild below can build the x86 closure under emulation in the same
# script invocation. On a fresh install where we have to use `boot`, the
...
if [ "$apple_silicon" = "y" ] && [ "$gaming" = "y" ] && \
  ...
  write_flake_nix "${modules_stage1[@]}"

  ok "Stage 1: nixos-rebuild $rebuild_action (binfmt registration only)"
  sudo nixos-rebuild "$rebuild_action" \
    --flake "$NIXOS_DIR#$hostname" \
    --option experimental-features "nix-command flakes" \
    "${rebuild_extra[@]}"

  # Restore the full flake (with gaming-asahi) so the stage-2 rebuild below
  # picks it up. Also leaves the on-disk flake correct for a post-reboot
  # `nixos-rebuild switch` if stage-1 used `boot`.
  write_flake_nix "${modules[@]}"

  if [ "$rebuild_action" = "boot" ]; then
    echo
    bold "Stage 1 staged (boot). Reboot required to activate qemu binfmt."
    echo "After reboot the gaming closure can build. Pick one:"
    echo "  - sudo nixos-rebuild switch --flake /etc/nixos#$hostname --impure"
    echo "  - Or re-run this script (it will skip the bootstrap automatically)."
    if ask_yn "Reboot now?" y; then sudo reboot; fi
    exit 0
  fi

  ok "Stage 1 active — binfmt live in running kernel. Continuing to stage 2."
fi
