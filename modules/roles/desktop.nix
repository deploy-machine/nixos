{ config, lib, pkgs, username, ... }:
{
  ...
  options.profile.autoLogin = lib.mkOption {
    type = lib.types.bool;
    default = config.boot.initrd.luks.devices != { };
    defaultText = lib.literalExpression "config.boot.initrd.luks.devices != { }";
    description = ''
      Start the user's Hyprland session straight from boot, skipping tuigreet.
      On by default only with an encrypted root: the LUKS passphrase or TPM
      PIN typed in the initrd already gates the machine, so a second login
      prompt adds typing, not security. hyprlock still guards idle/suspend.
    '';
  };
  ...
  config = {
    ...
    services.greetd = {
      ...
      settings = {
        ...
        # Boot straight into the session once per boot (greetd runs
        # initial_session only on its first start); logging out lands on
        # tuigreet as usual.
        initial_session = lib.mkIf config.profile.autoLogin {
